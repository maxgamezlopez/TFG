...
res['errorAbs'].plot.hist(bins=150, xlim=(-7000,7000), ylim=(0,20),color='#002e5d')
plt.ylabel('Frecuencia')
plt.xlabel('Error absoluto')
plt.show()

#Descripción analítica de error absouto
res['errorAbs'].describe()

#Representación de valores predichos frente a reales <<<Ilustración 7.30>>>
x = [1,30000]
y = [1,30000]
plt.subplot(121)
res.plot.scatter(y=['predicho'], x=['real'], figsize=(10,10), grid=True,s=1)
plt.plot(x,y, color="red", label = 'línea de referencia: tiempo previsto igual al de desplazamiento', linewidth=1 )
plt.title('Valores predichos frente a reales (árboles extremadamente aleatorios)')
...
joblib.dump(modelo, 'C:\\MYPATH\\TFG\\Python\\Modelos\\Modelo ExtraTree v1')

#El modelo puede ser cargado y empleado para la predicción de nuevos valores.
modelo = joblib.load('C:\\MYPATH\\TFG\\Python\\Modelos\\Modelo ExtraTree v1')

#Análisis de fechas de mayor error absoluto
por_fechas=pd.DataFrame()
por_fechas=res[res['predicho'] > 10000]
por_fechas['predicho']=por_fechas['predicho']+660
por_fechas['errorAbs']=por_fechas['predicho']-por_fechas['real']
por_fechas['errorAbs'].describe()
por_fechas.head()

por_fechas2['real']=y_val
por_fechas2['predicho']=modelo.predict(X_val)
por_fechas2['errorAbs']=por_fechas2['predicho']-por_fechas2['real']
por_fechas2['RECM']=((por_fechas2['errorAbs'])**2)**(1/2)
por_fechas2.head()

por_fechas2[ por_fechas2['RECM'] >3000]
